...
global photo_img
photo_img = tk.PhotoImage(file='photo_bg.gif')
...
def build_GUI(x_length,y_length):
    ...
    for c in range(y_length):
        ...
        for d in range(x_length):
            button = tk.Button(main,text='',bg='light grey',command=lambda row=c,column=d: reveal(row,column))
            button.img = photo_img
            button.configure(image=button.img,width=20,height=20,text='')
